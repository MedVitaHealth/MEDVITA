...
class CompletedVaccinationAdapter(private val onItemClick: (VaccinationModel) -> Unit) :
    ListAdapter<VaccinationModel, CompletedVaccinationAdapter.VaccinationViewHolder>(DiffCallback()) {
    ...
    inner class VaccinationViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        private val timeCompletedTextView: TextView = view.findViewById(R.id.time_remaining)

        @RequiresApi(Build.VERSION_CODES.O)
        fun bind(vaccination: VaccinationModel) {
            nameTextView.text = vaccination.name
            timeCompletedTextView.text = "Completed on ${vaccination.targetDate.toString()}"
        }
    }
